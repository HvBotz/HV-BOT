...
async def start(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "𝖴𝗉𝖽𝖺𝗍𝖾𝗌", url=invite_link.invite_link
                )
            ]
        ]
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            f_caption=CUSTOM_FILE_CAPTION.format(file_name= '' if title is None else title, file_size='' if size is None else size, file_captionInlineKeyboardButton('𝖴𝗉𝖽𝖺𝗍𝖾𝗌', url=f'https://t.me/HVCOMMUNITY')='' if f_caption is None else f_caption)
        except Exception as e:
            logger.exception(e)
            f_caption=f_caption
    if f_caption is None:
